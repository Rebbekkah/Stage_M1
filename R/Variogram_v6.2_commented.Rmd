---
title: "Scrit R for Data Viscosity Analysis"
author: "Claude Bostoen"
output: pdf_document
---

The program will be presented and organised in chuncks, that you can modify according to your needs.
You can have all your plots, code and/or outputs in a pdf format (knitr option) by modifying the chuncks inputs in the {r} brackets by following this link : https://rmarkdown.rstudio.com/articles_intro.html.
If you only want the plots to be generated you can change the text into the brackets into {r, echo = FALSE, fig.show = TRUE}.

The inputs are films which caracterize data viscosity at specific times and frequency.


Line 1281 start of programm
...
Here are the librairies you will need in order to run the program.
...
If you look closely to the program, the working directory isn't always the same depending on what you would do. Below are the paths you have to put to change the working directory.
...
```
For instance you are now on you generated files directory.

## Functions

In this chunck are separately loaded smaller functions that will be used later in others.
...
Variogram_experimental <- function(film, log_eta, Dimdata, simulation) {
  ...
  # Range fraction is also the choice parameter for variogram calculation and should be < 1
  ...
  dfvar <- data.frame(explag, variogram) # Make a dataframe before it writing into XLS
  ...
  # Pragmatic way to avoid variogram of delta eta not to be saved (usually FIT with a constant)
...
Distribution_check = function(eta, lognormal) { 
  ...
  # Cauchy only is always on deltas, therefore deltaeta = eta input/log normal so deltaeta = diff(eta)
  ...
  # Substract mean (obtained from gaussian) from deltaeta (normalization) and also for the master curve
  ...
  Param <- fitdistr(delta_eta, "t", start = starting_list, lower = lower) # t-Student checking
  ...
  if(lognormal == "false") {  # for the scaling law
    ...
    if(max(deltaeta_pos) > 10) {  # Decision criterion for the type of x axis (normalized to sigma or not)
      xlab = "delta T"
    } 
    else {
  ...
  else { # so if lognormal = true (datas are following a lognormal distribution) a plot will be generated for each films
...
# When Scaling function is called set "false" scaling is now done to sd and not the mean of the viscosity as earlier (however these are proportional to eachother)
# If statement has to be changed film = "filmx" first in the series of films
  ...
  # if(film == "film12" | film == "film13" | film == "film17" | film == "film10") { # As series 2
    ...
    # Series 1: film 9 and 21
    # Series 2: film 10 and 17
  ...
  if(lognormal == "false") {
    ...
    # Data have been scaled on individual basis (see above), therefore deltaeta_neg and pos are scaled
...
Scalingdelta_aeta = function() {   
  ...
  # If the datas are in series 3 : all false are putten
  ...
  if(FALSE) { # Film Series 2 for scaling
    File <-  c( "film10Scalingdeltaeta.xlsx", 
            "film12Scalingdeltaeta.xlsx",
            "film13Scalingdeltaeta.xlsx",
            "film14Scalingdeltaeta.xlsx", 
  ...
  #Here we check for the end of the file in order to determine the dimension of the data matrix --> Dimdata
  ...
  # Scaling law for all datas irrespectively of the frequency (one does not see this in litarature)
  # This part can be used to look at limited number files for the plot distribution data 
  # (FALSE) leads to analysis of limited number of data provided these have been set accordingly
  ...
  plot_datax <- NULL
  plotfile <- FALSE # plotfile <- TRUE for making one file, else 4 files .txt written with a cdf version
  ...
  for (i in 1:length(films)) {
    ...
    # To 2columns : x and y for fitdistr function
  ...
  delta_eta <- all_scaled_datax  #Delta_eta is the key variable and equal to all_scaled_datax
  ...
  # In case of series 3 --> all if are FALSE
...
Ouverture pr agrandir le programme
...
fit_general = function(film, fitlag, fitvariogram, averagetime, FIT, pos_line) {
  if (FIT == 0) {
    ...
    if(TRUE) {
      ...
      # Alse true if(film == "film12" | film == "film13" | film == "film17") {
  ...
  if (FIT == 1) {
    ...
    # Plot for publication
    if(film  ==  "film15" | film  ==  "film10" | film  ==  "film21") {
    # Also works if(film == "film12" | film == "film13" | film == "film17") {
  ...
  # Generalized Random Walk (GRW) mean reverting mechanism
  if (FIT == 2) {
  ...
  # Ornstein Uhlenbeck mean reverting mechanism (OUP)
  if (FIT  ==  3) {
  ...
  # (damped) cosinus - hast to be rewritten (constant names are constant in terms of seconds)
  if (FIT == 4) {  
...
For the reading of your data you have two choices : you can put all your data .txt in a folder (ONLY THEM) and use the first way to read them, which is great if you have a lot of samples.
Or if you want to choose which samples you want to analyze you can use the second way.

Note that the first method is in a commentary, so you have to get read of the "#" in order to use it, and put the second method as a commentary.

```{r, echo = TRUE, eval = FALSE}
first_reading_testing_rawdata = "no"   
# First_reading_testing_rawdata = "no"
# Set by default because data have been looked at already (just when there are new data)
...
# By default equidistant = "yes" (which make equidistant)
# Or "no" to treat raw data or already equidistant data
...
# rand = "OK"" for random sampling
# rand != "OK" so no randomization via sample (demonstration of the uniquen features of the time series results)
...
# Alternative way to read datafiles : by choosing them
alldatafiles <- NULL
...
for (i in 1:length(alldatafiles)) { ## mettre un commentaire : optimiser la boucle, lié au char string l1340
  ...
  if (characteristic_string == "run") { # In case of series 4
...
# Analysis settings that need to be checked
simulation <- "no" # no: reading of experimental data from excel file
variogram_theory <- "no"
make_equidistant <- "yes"
rand <- "K" # "OK" means randomize (all)
  ...
  if(simulation  ==  "no") {
    ...
    t_eta <-  read.delim(test, sep = "\t", nrows = 1000, header = TRUE, stringsAsFactors = FALSE)
      ...
      if(characteristic_string  ==  "film") {
        column <- 5   # The column number of your .txt file where the viscosity is written
    ...
    for (i_count in 1:1000) { 
      # Check the column number first in your .txt files for "viscosity (Pa s)" second series (4), first series (5)
...
legend_markers <- seq(1, length(film_number), by = 1)
...
if(TRUE) {
  ...
  plots.dir.paths  <- list.files(path_OutputAll, pattern = ".png", full.names = TRUE)
  plots.png.paths <- list.files(plots.dir.paths, pattern = ".png", full.names = TRUE)
  
  file.copy(from = plots.dir.paths, to = RStudio_path)  
  
  plots.png.details <- file.info(plots.dir.paths)
  print(plots.png.details)
  
  plots.png.details <- plots.png.details[order(plots.png.details$mtime, decreasing = TRUE),]
  print(plots.png.details)
  
  sorted.png.names = gsub(plots.dir.paths, RStudio_path,  
...
#file.remove(list.files(GF_path, include.dirs  = FALSE, full.names = TRUE, recursive = TRUE))
#file.remove(list.files(DAM_path, include.dirs = FALSE, full.names = TRUE, recursive = TRUE))
#file.remove(list.files(DAR_path, include.dirs = FALSE, full.names = TRUE, recursive = TRUE))
#file.remove(list.files(DAMD_path, include.dirs = FALSE, full.names = TRUE, recursive = TRUE))
#file.remove(list.files(RStudio_path, include.dirs = FALSE, full.names = TRUE, recursive = TRUE))
```
