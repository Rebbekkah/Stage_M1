...
setwd("/Users/rgoulanc/desktop/Rebecca/FAC/M1BI-IPFB/SEM2/Stage/BERRET_BOSTOEN/Fichiers/archive-3")
```
...
Variogram_theoretical <- function(film, Dimdata, averagetime, range_fraction, simulation, Fit, Variogram_fit) {
  ...
  if(range_fraction < 1) {
    range <- range_fraction * (Dimdata - 1)
    }
    setwd("/Users/macbookair/Desktop/Series/OutputAll/GeneratedFiles/")
    variogram_data <-  read_xlsx(paste(film, "variogram.xlsx", sep = ""), 
  ...
  if(max(fitvariogram) < mean(fitvariogram) + sd(fitvariogram)) {
    FIT <- FIT + 1
  }
  ...
  plot(fitlag, fitvariogram, xlab = xlab, ylab = "", type = "l",
       ylim = c(0, ymax), xlim = c(0, range), cex.lab = 1.5)
  ...
  if(simulation == "yes") {
    ...
    mtext(cex = 0.75, paste0("amplitude = ", amplitude, ", period = ", round(1/nu, 1),
                             " s, trend =  ", round(trend, 1)), 3, 0)
    pos_line <- 0
  }
...
Distribution_check = function(eta, lognormal) { 
  ...
  if(lognormal == "true") { 
    ...
    ecdf_rl <- rlnorm(1000, meanlog, sdlog)
    plot(ecdf_rl, cex.lab = 1.5, 
         col = "red", 
         add = TRUE, 
         do.points = FALSE)
    
    text <- paste("eta lognormal", round(etaecdf, 1), "hb", round(etaecdfhb, 1), "lb", round(etaecdflb, 1))
    mtext(cex = 0.75, text, 3, 0.5)
    deltaeta <- diff(eta) 
  }
...
points(deltaeta_pos, xpos_exp, pch = 20)
  ...
  if(lognormal == "true") {
    ...
    if(film == "film10") {
      write.table(tdistribution_fits, paste("tdistribution_fits.txt", sep= ""), col.names = TRUE, append = TRUE)
    }
    else {
  ...
  if(film == "film9" | film == "film15" | film == "film10" | film == "film21") {
    ...
    setwd("/Users/macbookair/Desktop/Series/Plots4Pub/DataAnalysisMethod/Dataprep/")
    ...
    setwd("/Users/macbookair/Desktop/Series/Plots4Pub/Results")
    if(film == "film9") {
      write.table(df_cdf_delta_eta, file = paste(film, "plot_cdf_delta_eta.txt", sep = ""),
                quote = FALSE, na = "", sep = " ", col.names = TRUE, row.names = FALSE, dec = ".")      
    }
...
Scalingdelta_aeta = function() {
  ...
  for (i in 1:length(films)) {
    ...
    if(plotfile) {
      ...
      for (index in 1:(nr-1)) {
        plot_index[[i]] <- c(plot_index[[i]], rep(NA, length(cdf_)))
      }
  ...
  tdist[1] <- t_general(min_eta, tm, ts, tdf)
  deltaeta_tstudent[1] <- min_eta
...
Rawdata_RDT = function(analyserawdata) {
  ...
  setwd("/Users/macbookair/Desktop/Series/InputData")
...
fit_general = function(film, fitlag, fitvariogram, averagetime, FIT, pos_line) {
  ...
  if(TRUE) {
    ...
    if(film == "film15" | film == "film10" | film == "film21") {
      ...
      setwd("/Users/macbookair/Desktop/Series/Plots4Pub/DataAnalysisMethod/")
...
ID_path <- "/Users/rgoulanc/desktop/Rebecca/FAC/M1BI-IPFB/SEM2/Stage/BERRET_BOSTOEN/Fichiers/archive-3"
print(ID_path)
Film_series <- NULL
...
for(i in 1:length(Film_series)) {
  ...
  if(simulation  ==  "yes") {
    ...
    if(characteristic_string  ==  "Lehigh") {
      eta <- 0.1 * head(unlist((t_eta[, column])), Dimdata)
    }
    ...
    delta <- max(t)/length(t)

    par(mfrow=c(2,4), xaxs="i", yaxs="i", mar = c(5,5,5,5))
    ...
    mtext(cex = 0.75, paste0("<eta> ", round(meaneta, 2), " sd ", round(sdeta, 1), 
                             " <eta>lin ", round(exp(meaneta), 2), " dt ", round(deltat, 2)), 3, 0)
    ##########
  ...
  ###chelou, le mettre dans le même if true?
  if(TRUE){
    test <- test_OK
  }
  ###
  ...
  if(film == "Lehigh_data_VolatilityClustering") {
    FIT <- FIT0
  }
  else {
    FIT <- FIT5
...
```{r}
path <- "/Users/macbookair/Desktop/Series/Plots4Pub/Results/"
...
plot(x_GRW_master_plot[1:range], y_GRW_master_plot[1:range], cex.lab = 1.5,
...
pub_master_x <- c("alfa^l", x_GRW_master_plot[1:range])
pub_master_y <- c(Film_frequencies[1],
                  y_GRW_master_plot[1:range], rep("0.0000", range*(film_number - 1)))
pub_line <- c("1-alfa^l", 1 - x_GRW_master_plot[1:range])
pub_master_GRW <- c(sprintf("%.6s", pub_master_y))

set_start <- i * range + 1
set_end <- i * range + range
...
for (i in 1:(film_number - 1)) {
  ...
  pub_master_y <- c(Film_frequencies[i + 1], rep("0.0000", set_start - 1),
                    y_GRW_master_plot[set_start:set_end], rep("0.0000", (film_number * range - set_end)))
...
plot(x_OUP_master_plot[1:range], y_OUP_master_plot[1:range], cex.lab = 1.5,
...
pub_master_y <- NULL
pub_line <- NULL
pub_master_x <- c("epsilon l", x_OUP_master_plot[1:range])
pub_master_y <- c(Film_frequencies[1], y_OUP_master_plot[1:range], 
                  rep("0.0000", range * (film_number - 1)))
pub_line <- c("1-exp(- epsilon l)", 1 - exp(-x_OUP_master_plot[1:range]))
...
for (i in 1:(film_number - 1)) {
  ...
  pub_master_y <- c(Film_frequencies[i + 1], rep("0.0000",set_start - 1),
                    ...
                    rep("0.0000", (film_number*range-set_end)))
...
plot(cex.lab = 1.5, Film_frequencies, Film_alfa, xlab = xlab, ylab = "", log = " xy",
     xlim = xlim, ylim = c(0.2, 2), pch = 17, col = "red")
title(ylab = y1lab, line = 2.5)
...
setwd("/Users/macbookair/Desktop/Series/OutputAll/GeneratedFiles/")
Fitoutput_path <- "/Users/macbookair/Desktop/Series/OutputAll/"
file.copy(from <- list.files(pattern = "Fit Output.txt", full.names = TRUE), overwrite = TRUE,
          to = Fitoutput_path)

if(TRUE) {
  RStudio_path <- "/Users/macbookair/Desktop/Series/OutputAll/ScreenPlotsRStudio/"
...
GF_path <- "/Users/macbookair/Desktop/Series/OutputAll/GeneratedFiles/"
RStudio_path <- "/Users/macbookair/Desktop/Series/OutputAll/ScreenPlotsRStudio/"
DAM_path <- "/Users/macbookair/Desktop/Series/Plots4Pub/DataAnalysisMethod/"
DAMD_path <- "/Users/macbookair/Desktop/Series/Plots4Pub/DataAnalysisMethod/Dataprep/"
DAR_path <- "/Users/macbookair/Desktop/Series/Plots4Pub/Results/"
