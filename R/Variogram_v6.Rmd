...
fit_general = function(film, fitlag, fitvariogram, averagetime, FIT, pos_line) {
  ...
  if (FIT == 1) {
    ...
    variogramfit <- nlsLM(fitvariogram ~ Modelhole(fitlag, C0_constant_cosinus, 
  ...
  if (FIT == 2) {
    ...
    variogramfit <- nlsLM(fitvariogram ~ Modelhole(fitlag, C0_constant_GRW, C1_GRW),
  ...
  if (FIT  ==  3) {
    ...
    variogramfit <- nlsLM(fitvariogram ~ Modelhole(fitlag, C0_constant_OUP, C1_correlationtime),
  ...
  if (FIT == 4) {  
    ...
    variogramfit <- nlsLM(fitvariogram ~ Modelhole(fitlag, C0, C1, C2),
  ...
  if (FIT == 5) {
    ...
    variogramfit <- nlsLM(fitvariogram ~ Modelhole(fitlag, C0),
                          start = list(C0 = 1))
    C0 = coef(variogramfit)[1]
    lines(predict(variogramfit), col = 'red')
...
############Alternative way to read datafiles
alldatafiles <- NULL
...
for(i in 1:length(Film_series)) {
    ...
    if(variogram_theory == "yes") {
      ...
      for (i in 2:nr) {
        x[i] <- (alfa*x[i-1] + amplitude * (cos(6.28*nu*i) - cos(6.28*nu*(i-1))) +
                   ksi_previous-average_viscosity + trend)
      ...
      plot(t, x, cex.lab = 1.5, type = "l",ylim = c(min(x), max(x) + average_viscosity), xlim = c(0,max(at)))
      mtext(cex = 0.75, paste0("amplitude = ", amplitude, ", period = ", round(period, 1),
                               " s, trend =  ", round(trend, 1)), 3, 0)
  ...
  if(test == "KO" | rand == "OK") {
    ...
    ###############si fit2 marche jusqu'au film19 mais c'est faux
